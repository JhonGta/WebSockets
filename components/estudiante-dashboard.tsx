...
export function EstudianteDashboard({ user }: EstudianteDashboardProps) {
  ...
  // Ejemplo de función para registrar equipo y emitir evento
  const handleRegisterEquipo = () => {
    // Aquí iría la lógica real de registro en la base de datos
    const data = {
      nombreEstudiante: user.name,
      equipo: "PC-05" // Reemplazar con el equipo real seleccionado
    }
    socket.emit("equipo:registrado", data)
    alert(`Equipo registrado: ${data.equipo}`)
  }
                      ...
                      {practicas.map((practica) => (
                        <tr key={practica.id} className="border-t">
                          ...
                          <td className="px-4 py-2">
                            ...
                            {/* Botón para registrar equipo y emitir evento */}
                            <Button size="sm" className="ml-2" onClick={handleRegisterEquipo}>
                              Registrar Equipo
                            </Button>
                          </td>
                        </tr>
